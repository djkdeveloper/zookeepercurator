...
public class ZookeeperCrud {
    ...
    public static void main(String[] args) throws Exception {
        ...
        client.getCuratorListenable().addListener((cc, event) -> {
            System.out.println(event.getType());
            System.out.println(event.getPath());
            System.out.println("=====================================");
        });

        createNodeWithCallback(client,(x,y)->{
            System.out.println(y.getPath());
        }, "/djkcccc", "d".getBytes());

        createNodeAsync(client,"/djktttt","d".getBytes());
     ...
     * @param payload 节点数据
     */
    public static void createNodeAsync(CuratorFramework client, String path,
                                       byte[] payload) throws Exception {
        client.create().inBackground().forPath(path, payload);
    }
     ...
     * @throws Exception
     */
    public static void createNodeWithCallback(CuratorFramework client, BackgroundCallback callback, String path,
                                              byte[] payload) throws Exception {
        client.create().inBackground(callback).forPath(path, payload);
    }
